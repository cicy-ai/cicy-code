...
// conpty_windows.go — vendored from github.com/UserExistsError/conpty v0.1.4
// (MIT License, Copyright (c) UserExistsError). Vendored to SPLIT the close
// sequence: the upstream Close() does ClosePseudoConsole + CloseHandle×6 in
// one shot while other goroutines may still be blocked in ReadFile /
// WaitForSingleObject on those handles — Windows reuses freed handle slots,
// so the pending I/O then lands on a FOREIGN handle and silently corrupts /
// kills the whole process (observed killing the server on a real box).
// Split API: ClosePty() terminates the console (readers drain with
// BROKEN_PIPE, waiters see process exit), ReleaseHandles() frees the handles
// strictly after all I/O users are done.
...
// ClosePty terminates the pseudo console (and with it the attached client).
// Pending ReadFile calls drain with BROKEN_PIPE/EOF; Wait() observes the
// process exit. Handles stay valid until ReleaseHandles().
func (cpty *ConPty) ClosePty() {
	// there is no return code
	win32ClosePseudoConsole(cpty.hpc)
}

// ReleaseHandles frees all handles. Call EXACTLY ONCE, strictly after every
// Read/Write/Wait user has finished — never while I/O may still be pending.
func (cpty *ConPty) ReleaseHandles() error {
	return closeHandles(
		cpty.pi.Process,
		cpty.pi.Thread,
		cpty.ptyIn.handle,
...
// Close keeps upstream-compatible behavior for single-threaded callers.
func (cpty *ConPty) Close() error {
	cpty.ClosePty()
	return cpty.ReleaseHandles()
}
