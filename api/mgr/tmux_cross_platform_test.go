...
func TestAgentBootLinesKiroUsesLiveInstallAndStartsChat(t *testing.T) {
	lines := agentBootLines("kiro-cli", false, false, "w-10001", "")
...
func TestAgentBootLinesKiroWritesChineseReplySteeringWhenEnabled(t *testing.T) {
	lines := agentBootLines("kiro-cli", true, true, "w-10001", "")
	script := strings.Join(lines, "\n")
...
func TestAgentBootLinesCopilotStartsYoloAndTrustsWorkspace(t *testing.T) {
	for _, allowAllActions := range []bool{false, true} {
		lines := agentBootLines("copilot", allowAllActions, false, "w-10005", "")
		script := strings.Join(lines, "\n")
...
func TestAgentBootLinesHermesUsesHomeLogExprWithoutNestedQuotes(t *testing.T) {
	lines := agentBootLines("hermes", false, false, "w-10009", "")
	script := strings.Join(lines, "\n")
	if !strings.Contains(script, `export CICY_HERMES_MODEL='gpt-5.5'`) {
		t.Fatalf("hermes boot lines should default to gpt-5.5: %s", script)
	}
...
func TestAgentBootLinesOpenCodeDoesNotUseUnsupportedPermissionFlag(t *testing.T) {
	lines := agentBootLines("opencode", true, true, "w-10004", "")
	script := strings.Join(lines, "\n")
...
func TestAgentBootLinesOpenCodeWithoutChineseReplyRemovesInstructionsFile(t *testing.T) {
	lines := agentBootLines("opencode", true, false, "w-10004", "")
	script := strings.Join(lines, "\n")
	if strings.Contains(script, `"instructions": ["${WORKSPACE}/.opencode/reply-in-chinese.md"]`) {
...
func TestAgentBootLinesClaudeWritesSettingsFile(t *testing.T) {
	lines := agentBootLines("claude", false, false, "w-10001", "")
	script := strings.Join(lines, "\n")
	if strings.Contains(script, `node - "$WORKSPACE" <<'EOF'`) {
		t.Fatalf("claude boot lines should not mutate ~/.claude.json via node: %s", script)
	}
...
func TestAgentBootLinesClaudeAllowAllActionsUsesDangerousSkipPermissions(t *testing.T) {
	claudeScript := strings.Join(agentBootLines("claude", true, false, "w-10001", ""), "\n")
	...
	cicyClaudeScript := strings.Join(agentBootLines("cicy-claude", true, false, "w-10001", ""), "\n")
...
func TestInitPaneEnvScriptBootstrapsCicyTmuxConf(t *testing.T) {
	...
	lines = append(lines, agentBootLines("codex", false, false, shortID, "")...)
