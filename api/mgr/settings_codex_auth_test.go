...
func TestHandleCodexAuthImportRestoresDecodedJSON(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	...
	handleCodexAuthImport(recorder, httptest.NewRequest(http.MethodPost, "/api/settings/codex-auth", strings.NewReader(body)))
...
func TestHandleCodexAuthImportRejectsInvalidInputWithoutChangingAuth(t *testing.T) {
	tests := []struct {
		name string
		body string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			recorder := httptest.NewRecorder()
			handleCodexAuthImport(recorder, httptest.NewRequest(http.MethodPost, "/api/settings/codex-auth", strings.NewReader(tt.body)))
