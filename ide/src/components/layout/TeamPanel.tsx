...
export default function TeamPanel({ paneId, token }: Props) {
  ...
  const [confirmUnbind, setConfirmUnbind] = useState<Binding | null>(null);
    ...
    <div className="h-full flex flex-col">
      ...
      {confirmUnbind && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-[#1e1e1e] border border-[var(--vsc-border)] rounded-lg p-4 mx-4 max-w-xs w-full shadow-xl">
            <p className="text-sm text-zinc-300 mb-4">
              Unbind <span className="text-zinc-100 font-medium">{getName(shortId(confirmUnbind.name))}</span>?
            </p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setConfirmUnbind(null)} className="text-sm px-3 py-1.5 rounded bg-white/[0.06] text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.1] transition-colors cursor-pointer">Cancel</button>
              <button onClick={() => { unbind(confirmUnbind); setConfirmUnbind(null); }} className="text-sm px-3 py-1.5 rounded bg-red-500/20 text-red-400 hover:bg-red-500/30 transition-colors cursor-pointer">Unbind</button>
            </div>
          </div>
        </div>
      )}
