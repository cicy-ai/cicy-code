    ...
    <div className="h-full flex flex-col bg-vsc-bg-secondary">
      ...
      <div className="h-10 flex items-center justify-between px-3 border-b border-vsc-border flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-vsc-text-secondary">
            {counts.thinking ? <span className="text-yellow-500">{counts.thinking}⚡</span> : null}
            {counts.thinking && counts.idle ? ' ' : null}
            {counts.idle ? <span className="text-green-500">{counts.idle}✓</span> : null}
          </span>
        </div>
        <button onClick={() => openDialog('createAgent')} className="p-1 rounded text-vsc-text-secondary hover:text-vsc-text hover:bg-vsc-bg-hover" title="New Master">
      ...
      {/* Role filter */}
      <div className="flex px-2 pb-1 gap-1 flex-shrink-0">
        {(['master', 'worker', 'all'] as const).map(f => (
          <button key={f} onClick={() => setRoleFilter(f)}
            className={`px-2 py-0.5 text-[11px] rounded ${roleFilter === f ? 'bg-vsc-button text-white' : 'text-vsc-text-secondary hover:text-vsc-text hover:bg-vsc-bg-hover'}`}>
        ...
        {filtered.map((pane: any) => {
          ...
          return (
            <div key={pane.pane_id}
              ...
              className={`group flex items-center gap-2 px-3 py-1.5 cursor-pointer border-l-2 transition-colors ${isThinking ? 'bg-yellow-500/20 border-l-yellow-500' : isActive ? 'bg-blue-500/15 border-l-blue-500' : isPinned ? 'bg-vsc-bg-hover border-l-vsc-text-secondary' : 'border-l-transparent hover:bg-vsc-bg-hover'}`}>
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${si.color}`} title={si.label} />
              {/* Content */}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1">
                  {isPinned && <span className="text-[9px] leading-none">📌</span>}
                  <span className={`text-[13px] truncate block ${isActive ? 'text-vsc-text font-medium' : 'text-vsc-text-secondary'}`}>{title}</span>
                  <span className="text-[10px] text-vsc-text-secondary ml-auto flex-shrink-0">{pane.timeAgo != null ? formatTimeAgo(pane.timeAgo) : ''}</span>
                </div>
                <div className="flex items-center gap-1.5 text-[10px] text-vsc-text-secondary">
                  <span>{shortId}</span>
                  {si.label && si.label !== 'idle' && (
                    <span className={`px-1 rounded ${isThinking ? 'text-yellow-400' : si.label === 'auth' ? 'text-red-400' : 'text-vsc-text-secondary'}`}>{si.label}</span>
                  )}
                  {ctxPct != null && (
                    <div className="flex items-center gap-1 ml-auto">
                      <div className="w-12 h-1.5 bg-vsc-bg rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${ctxPct > 80 ? 'bg-red-500' : ctxPct > 50 ? 'bg-yellow-500' : 'bg-green-500'}`} style={{ width: `${Math.min(ctxPct, 100)}%` }} />
                      </div>
                      <span className="text-[9px]">{ctxPct}%</span>
                    </div>
                  )}
                </div>
              </div>
