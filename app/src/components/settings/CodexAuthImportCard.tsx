...
export default function CodexAuthImportCard() {
  ...
  const restore = async () => {
    ...
    const accepted = await confirm({
      title: t('settingsCodexAuthConfirmTitle', { defaultValue: '覆盖 Codex Auth？' }),
      body: t('settingsCodexAuthConfirmBody', { defaultValue: '这会用输入内容覆盖 ~/.codex/auth.json。已运行的 Codex 不受影响，之后启动的 Codex 使用新凭据。' }),
      danger: true,
      confirmLabel: t('settingsCodexAuthConfirm', { defaultValue: '确认覆盖' }),
    ...
    setMessage(null);
    try {
      await apiService.importCodexAuth(encoded);
      setValue('');
      setShown(false);
      setMessage({
        kind: 'ok',
        text: t('settingsCodexAuthSuccess', { defaultValue: '已覆盖，之后启动的 Codex 会使用新凭据。' }),
      });
    } catch (error: any) {
      setMessage({
        kind: 'error',
        text: error?.response?.data?.detail || t('settingsCodexAuthFailed', { defaultValue: '覆盖失败，请确认输入的是有效的 Codex Auth Base64。' }),
      });
    } finally {
      setSaving(false);
    }
  ...
  return (
    <>
      <section data-id="settings-codex-auth-block" className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[13px] font-semibold text-zinc-100">
              {t('settingsCodexAuthTitle', { defaultValue: 'Codex Auth' })}
            </div>
            <div className="mt-0.5 text-[11px] leading-5 text-zinc-500">
              {t('settingsCodexAuthHint', { defaultValue: '粘贴 Codex Auth Base64，解码校验后覆盖 ~/.codex/auth.json。' })}
            </div>
            <div className="mt-3 flex items-center gap-2">
              <input
                data-id="settings-codex-auth-input"
                aria-label="Codex Auth Base64"
                ...
                placeholder={t('settingsCodexAuthPlaceholder', { defaultValue: '粘贴 Base64 内容' })}
                autoComplete="off"
                spellCheck={false}
              ...
              />
              <button
                data-id="settings-codex-auth-toggle"
                type="button"
            ...
            <div className="mt-3 flex items-center gap-2">
              <button
                data-id="settings-codex-auth-restore"
                type="button"
              ...
              >
                {saving ? t('settingsCodexAuthRestoring', { defaultValue: '覆盖中…' }) : t('settingsCodexAuthRestore', { defaultValue: '还原并覆盖' })}
              </button>
              {message ? (
                <span data-id="settings-codex-auth-message" className={`flex items-center gap-1 text-[11px] ${message.kind === 'ok' ? 'text-emerald-400' : 'text-rose-400'}`}>
