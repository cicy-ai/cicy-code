...
const ToolCard = ({ tool, running }: { tool: any; running?: boolean }) => {
  ...
  return (
    <div className={`rounded-lg bg-[#1a1a2e]/60 border ${borderColor} overflow-hidden`}>
      <div className="flex items-center gap-2 px-3 py-1.5 cursor-pointer select-none hover:bg-white/[0.03] transition-colors" onClick={() => setOpen(p => !p)}>
      ...
      {open && arg && <div className="px-3 py-1.5 text-sm font-mono text-vsc-text/50 whitespace-pre-wrap break-all border-b border-white/[0.04]">{arg}</div>}
      {open && hasDiff ? (
        <div className="mx-2 mb-2 rounded overflow-hidden border border-white/[0.06] text-xs font-mono max-h-[300px] overflow-auto">
          {tool.diff.old && tool.diff.old.split('\n').map((line: string, i: number) => <div key={`o${i}`} className="px-2 bg-red-500/[0.08] text-red-400/80 whitespace-pre-wrap break-all leading-relaxed">- {line}</div>)}
          {tool.diff.new && tool.diff.new.split('\n').map((line: string, i: number) => <div key={`n${i}`} className="px-2 bg-emerald-500/[0.08] text-emerald-400/80 whitespace-pre-wrap break-all leading-relaxed">+ {line}</div>)}
        </div>
      ) : open && tool.result ? (
        <pre className={`text-xs mx-2 mb-2 px-2.5 py-1.5 rounded font-mono whitespace-pre-wrap break-all max-h-[200px] overflow-auto leading-relaxed ${isError ? 'bg-red-500/[0.06] text-red-400' : 'bg-emerald-500/[0.04] text-emerald-400'}`}>{tool.result}</pre>
      ) : null}
    </div>
  )
}
    ...
    <div data-id="chat-history-view" className="flex flex-col h-full">
              ...
              <div data-id="chat-history-turn" key={gi} className="mb-5" ref={isLatest ? latestQRef : undefined}>
                ...
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                  <div className="flex items-center gap-1.5 px-3.5 py-1 border-b border-white/[0.03] flex-wrap">
                    <span className="text-vsc-accent text-sm font-medium opacity-60">✦ AI</span>
                  ...
                  <div className="px-3.5 py-2.5">
                    {steps.map((s: any, si: number) => {
                      ...
                      if (s.type === 'text') {
                        ...
                        return <div key={si} className={`chat-markdown text-base text-vsc-text leading-[1.7] ${si > 0 ? 'mt-2 pt-2 border-t border-white/[0.04]' : ''}`}><Markdown remarkPlugins={[remarkGfm]}>{s.text}</Markdown></div>
                      }
      ...
      <div data-id="chat-history-sticky-bar" className="shrink-0 border-t border-white/[0.06] bg-[#0f1014] px-3 py-2">
          <div className="flex items-center gap-3 rounded-lg border border-white/[0.06] bg-white/[0.03] px-3 py-2">
            {stickyWorking ? (
              <>
                <div className={`h-2 w-2 rounded-full ${(liveStatus === 'streaming' || lastTurnStatus === 'streaming') ? 'bg-blue-400 animate-pulse' : 'bg-yellow-400 animate-pulse'}`} />
                <div className="text-sm text-zinc-200">{stickyLabel}</div>
              </>
            ) : null}
            {suggestionText ? (
              <>
                <div className="min-w-0 flex-1 text-sm text-zinc-200 whitespace-pre-wrap break-words">{suggestionText}</div>
                <button
                  type="button"
