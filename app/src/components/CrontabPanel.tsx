    ...
    <div data-id="crontab-panel" className="flex h-full flex-col bg-[#0b0b0d] text-zinc-200">
      ...
      <div className="min-h-0 flex-1 overflow-auto p-4">
        ...
        <div className="space-y-2">{jobs.map((job, index) => <div key={job.id} data-id={`crontab-job-${index}`} className={`rounded-xl border p-3 ${job.enabled ? 'border-white/[0.08] bg-white/[0.025]' : 'border-white/[0.05] bg-black/20 opacity-60'}`}>
          <div className="mb-2 flex items-center justify-between"><span className="text-xs font-medium text-zinc-400">任务 {index + 1}</span><div className="flex items-center gap-2"><button onClick={() => update(job.id, { enabled: !job.enabled })} className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] ${job.enabled ? 'bg-emerald-500/15 text-emerald-300' : 'bg-white/[0.05] text-zinc-500'}`}>{job.enabled ? <Check className="h-3 w-3" /> : null}{job.enabled ? '已启用' : '已停用'}</button><button onClick={() => { setJobs((v) => v.filter((x) => x.id !== job.id)); setDirty(true) }} className="rounded p-1.5 text-zinc-600 hover:bg-red-500/10 hover:text-red-400"><Trash2 className="h-3.5 w-3.5" /></button></div></div>
