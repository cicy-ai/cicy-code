...
function Panel({ stores, onClose }: { stores: Record<string, { state: Record<string, any>; setter?: any }>; onClose: () => void }) {
  ...
  return (
    <div data-id="dev-panel" className="fixed z-[999999] flex flex-col bg-[#111113] border border-white/[0.1] rounded-xl shadow-2xl overflow-hidden select-none"
      style={{ left: pos.x, top: pos.y, width: size.w, height: size.h }}>
      {/* Header */}
      <div data-id="dev-panel-header" className="flex items-center gap-2 px-3 py-2 bg-[#0d0d0f] border-b border-white/[0.06] cursor-move shrink-0" onMouseDown={onDragStart}>
        <Bug className="w-3.5 h-3.5 text-purple-400" />
        <span data-id="dev-panel-title" className="text-xs font-semibold text-purple-300">{t('title')}</span>
        <span data-id="dev-panel-stores-count" className="text-[10px] text-zinc-600 ml-1">{t('storesCount', { count: storeNames.length })}</span>
        <div data-id="dev-panel-spacer" className="flex-1" />
        <input data-id="dev-panel-filter" value={filter} onChange={e => setFilter(e.target.value)} placeholder={t('filterPlaceholder')}
        ...
        <button data-id="dev-panel-close" onClick={onClose} className="p-1 rounded hover:bg-white/[0.06] text-zinc-500 hover:text-zinc-300 cursor-pointer">
          <X className="w-3.5 h-3.5" />
        </button>
      </div>

      {/* Body */}
      <div data-id="dev-panel-body" className="flex-1 overflow-y-auto text-[12px] font-mono">
        {storeNames.map(name => (
        ...
        {storeNames.length === 0 && (
          <div data-id="dev-panel-empty" className="p-4 text-zinc-600 text-center text-[11px]">{t('noStores')}</div>
        )}
      </div>

      {/* Resize handle */}
      <div data-id="dev-panel-resize-handle" className="absolute bottom-0 right-0 w-4 h-4 cursor-se-resize" onMouseDown={onResizeStart}>
...
function StoreSection({ name, state, hasSetter, filter, expanded, toggle }: {
  ...
  return (
    <div data-id={`dev-panel-store-${name}`} className="border-b border-white/[0.04]">
      <button data-id={`dev-panel-store-${name}-toggle`} onClick={() => toggle(name)}
        ...
        <span data-id={`dev-panel-store-${name}-name`} className="text-purple-400 font-semibold">{name}</span>
        <span data-id={`dev-panel-store-${name}-count`} className="text-zinc-700 text-[10px] ml-auto">{t('itemsCount', { count: entries.length })}</span>
      </button>
      {isOpen && (
        <div data-id={`dev-panel-store-${name}-body`} className="pb-1">
          {filtered.map(([key, val]) => (
            <ValueRow key={key} storeName={name} path={key} value={val} hasSetter={hasSetter} depth={0} />
          ))}
          {filtered.length === 0 && <div data-id={`dev-panel-store-${name}-no-match`} className="px-6 py-1 text-zinc-700 text-[10px]">{t('noMatch')}</div>}
        </div>
      )}
    </div>
  );
...
function ValueRow({ storeName, path, value, hasSetter, depth }: {
  ...
  return (
    <>
      <div data-id={`dev-panel-row-${storeName}-${path}`} className="group flex items-center gap-1 px-3 py-[3px] hover:bg-white/[0.03]" style={{ paddingLeft: pl }}>
        {isExpandable ? (
          <button data-id={`dev-panel-row-${storeName}-${path}-toggle`} onClick={() => setOpen(!open)} className="p-0 cursor-pointer">
            {open ? <ChevronDown className="w-3 h-3 text-zinc-600" /> : <ChevronRight className="w-3 h-3 text-zinc-600" />}
          </button>
        ) : <span data-id={`dev-panel-row-${storeName}-${path}-spacer`} className="w-3" />}

        <span data-id={`dev-panel-row-${storeName}-${path}-key`} className="text-zinc-400 shrink-0">{path.split('.').pop()}</span>
        <span data-id={`dev-panel-row-${storeName}-${path}-sep`} className="text-zinc-700 mx-0.5">:</span>

        {editing ? (
          <input data-id={`dev-panel-row-${storeName}-${path}-input`} autoFocus value={editVal} onChange={e => setEditVal(e.target.value)}
          ...
          <span data-id={`dev-panel-row-${storeName}-${path}-value`} className={`truncate ${colorClass}`} title={String(value)}>{displayVal}</span>
        )}

        <div data-id={`dev-panel-row-${storeName}-${path}-actions`} className="ml-auto flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
          {!isFunc && !isExpandable && (
            <button data-id={`dev-panel-row-${storeName}-${path}-copy`} onClick={copy} className="p-0.5 rounded hover:bg-white/[0.08] cursor-pointer" title={t('copy')}>
              {copied ? <Check className="w-3 h-3 text-green-400" /> : <Copy className="w-3 h-3 text-zinc-600" />}
            </button>
          )}
          {hasSetter && !isFunc && !isExpandable && (
            <button data-id={`dev-panel-row-${storeName}-${path}-edit`} onClick={startEdit} className="p-0.5 rounded hover:bg-white/[0.08] cursor-pointer" title={t('edit')}>
