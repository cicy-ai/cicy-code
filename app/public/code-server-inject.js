...
(function(){
  ...
  function connectChatBridge() {
    if (!token || !pageClientId || !pagePaneId || ws) return;
    try {
      var proto = window.location.protocol === 'https:' ? 'wss://' : 'ws://';
      ws = new WebSocket(proto + window.location.host + '/api/chat/ws?agent_id=' + encodeURIComponent(pagePaneId) + '&token=' + encodeURIComponent(token) + '&client_id=' + encodeURIComponent(codeClientId));
      ws.onopen = function() {
        try {
          ws.send(JSON.stringify({ type: 'code.ready', data: { page_client_id: pageClientId, code_client_id: codeClientId } }));
        } catch (e) {}
      };
      ws.onmessage = function(event) {
        try {
          var msg = JSON.parse(String(event.data || ''));
          if (msg && msg.type === 'host.open_file' && msg.data && msg.data.path) {
            console.log('[code-server-inject] host.open_file', msg.data.path);
          } else if (msg && msg.type === 'host.ping') {
            ws.send(JSON.stringify({
              type: 'code.pong',
              data: {
                requestId: String((msg.data && msg.data.requestId) || ''),
                page_client_id: pageClientId,
  ...
  function sendPathToPageClient(path) {
    if (!ws || ws.readyState !== WebSocket.OPEN || !path) return;
    try {
      ws.send(JSON.stringify({
        type: 'code.send_path',
        data: {
